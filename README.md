...
Assume a function $`f(x)=x^2`$. Assume a set of $`K`$ lists, $`X_{1},X_{2},...X_{K}`$, where each list $`X_{i}`$ has $`N_{i}`$ elements. So, the lists do not necessarily have the same lengths. The function that needs to be maximized is $`O = (\sum_{k=1}^{K}f(x_{k})) \mod M`$, where each $`x_{i}`$ is one element picked from list $`X_{i}`$. For this assignment you can assume that all the elements from the lists are larger or equal to $`1`$ or smaller or equal to $`10^9`$. That is, $`1 \leq x_{ik} \leq 10^9`$.

To give an example, assume that we have a 3 lists: `([5, 4], [7, 8, 9], [5, 7, 8, 9, 10])`, and that $`M=40`$. Then the maximum value of $`O`$ is 37, and is achieved by choosing 4 from the first list, 9 from the second and 10 from the third list: $`37=(4^2+9^2+10^2)\mod 40`$. Note that because of the properties of the modulo operator, this problem cannot be solved by just taking the largest element of each list.
